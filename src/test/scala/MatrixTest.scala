import DataStructures.SlopeOneDeviation._
...
class MatrixTest extends FlatSpec with Matchers {
  ...
  "Matrix" should "be filled" in {
    ...
    val matrix = (Matrix() /: items.flatMap(i => items.map(j => (i, j, slopeOne(i, j, users)))))((r, c) => r.addDeviation(c._1, c._2, c._3))
